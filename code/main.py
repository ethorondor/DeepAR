...
#%%
data_in = data[:data.shape[0] - (predictSize+windowSize), :]
#%%
for i in range(data_in.shape[1]):
    if i == 0:
        d1 = np.array(data_in)[:,i][:,np.newaxis]
        output = [np.array(series_to_supervised(d1))]
    else:
        d1 = np.array(data_in)[:,i][:,np.newaxis]
        out = np.array(series_to_supervised(d1))
...
#%%
data_train = output.reshape(input_data.shape[0]*input_data.shape[1],predictSize+windowSize)
#%%
data_train = input_data.reshape(input_data.shape[0]*input_data.shape[1],predictSize+windowSize)
#%%
