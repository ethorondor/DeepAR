...
######################################################################################
# get covariates: the covariates are month, weekday and hour indicators ##############
######################################################################################
# %%
times = data_frame[train_start:test_end].index
...
#find the position of first non zero value for each series
...
#1+num_covariates+1: ground truth+age+day of week+hour of day+month of year+num_series
x_input = np.zeros((total_windows,window_size,1+num_covariates+1),dtype='float32')
label = np.zeros((total_windows,window_size),dtype='float32')
v_input = np.zeros((total_windows,window_size),dtype='float32')
#%%
count = 0
if not train:
    covariates = covariates[-time_len:]
for series in trange(num_series):
    #create standarized age covariate
    ...
    for i in range(windows_per_series[series]):
        ...
        '''
        #create one period of lag for the series itself
        x_input[count, 1:, 0] = data[window_start:window_end-1, series]
        #4 contemporary covariates
        x_input[count, :, 1:1+num_covariates] = covariates[window_start:window_end, :]
        #the index of time series, there are 370 series
        x_input[count, :, -1] = series
        #the target variable, we are trying to forecast one period ahead time series with contemorary covariates
